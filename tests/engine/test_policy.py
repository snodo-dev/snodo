...
# operational error (error=True) and halts fail-closed; it is never excluded
# from the counts or converted into a pass.
...
def test_no_verdict_error_halts_every_policy(policy):
    """A judge that produced no verdict is an error; no policy can escape it."""
    evaluator = PolicyEvaluator()
    results = [
        make_result("v1", "pass"),
        make_result("v2", "pass"),
        ValidatorResult(
            validator_id="v3",
            severity="blocker",
            justification="Validator did not return a verdict. Fail-closed.",
            error=True,
        ),
    ]
...
def test_error_is_never_counted_as_a_pass():
    """An operational error moves no pass count — it is never a pass vote."""
    evaluator = PolicyEvaluator()
    err = ValidatorResult(validator_id="v1", severity="blocker",
                          justification="no verdict", error=True)
    results = [make_result("v2", "pass"), err]
