...
def blocker_completion_fn():
    """Return a completion function that always yields a 'blocker' verdict."""
    msg = MagicMock()
    msg.content = '{"severity": "blocker", "justification": "critical issue"}'
    response = MagicMock()
    response.choices = [MagicMock(message=msg)]
    return MagicMock(return_value=response)
...
def validation_passing(server):
    """Patch the validator LLM so validate_task returns 'pass'."""
    with patch(
