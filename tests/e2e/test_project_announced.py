...
def test_project_announced_again_on_second_run(snodo_cli):
    """A second run in the same project re-announces the project.

    A run reuses the active session, so the second run must start a new
    session (delete the first) to exercise the re-announcement path.
    """
    tmp_dir = snodo_cli.home
    subprocess.run(
        ["git", "remote", "add", "origin", "git@github.com:myorg/myrepo.git"],
    ...
    r_del = snodo_cli(["session", "delete", session_id])
