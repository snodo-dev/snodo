...
def _subprocess_coder_for_model(model: str) -> Optional[tuple[str, Any]]:
    from snodo.cli.commands.models_check import _subprocess_coder_for_model
    coder_name = _subprocess_coder_for_model(model)
    return (coder_name, None) if coder_name is not None else None
...
# ============================================================================
# --set-baseline: keep a local quality reference for one plan task
# ============================================================================

def _baseline_safe_component(value: Optional[str]) -> Optional[str]:
...
def _baseline_task_job(project_root: Path, plan: str, task: str) -> Optional[dict]:
    """Find the completed child job that recorded *plan*/*task*.

    Plan-run task state is deliberately not used here: it is a liveness/status
    record. The child job's task.json and state.json are the durable run record,
    and the parent job relationship prevents a similarly named task in another
    plan from being selected.
    """
    jobs_dir = project_root / ".snodo" / "jobs"
    candidates = []
    if not jobs_dir.is_dir():
    ...
    for job_dir in jobs_dir.iterdir():
        ...
        if task_plan == plan:
            candidates.append({"id": job_dir.name, "task": task_data, "state": state})
...
def _baseline_merge_sha(project_root: Path, plan: str, task: str) -> Optional[str]:
    """Return the recorded merge commit for this plan task, when available."""
    try:
        from snodo.infrastructure.audit import AuditLog
        audit = AuditLog(str(project_root / ".snodo" / "audit.log"))
...
def _baseline_git_snapshot(
    project_root: Path, change_size: dict, merge_sha: Optional[str] = None
) -> str:
...
def models_set_baseline_command(args) -> int:
    ...
    plan = _baseline_safe_component(getattr(args, "plan", None))
    task = _baseline_safe_component(
        getattr(args, "task", None) or getattr(args, "task_id", None)
    )
    ...
    job = _baseline_task_job(root, plan, task)
    if not job:
    ...
    try:
        diff = _baseline_git_snapshot(root, change_size, _baseline_merge_sha(root, plan, task))
    except Exception as exc:
