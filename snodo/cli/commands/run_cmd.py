...
from snodo.cli.commands.task_record import (  # noqa: F401
    _record_task_start,
...
def _auto_merge_block_reason(protocol, mode, closure_tree, worktree_path_val, worktree_degraded, project_root: Optional[str] = None, task: Optional[Any] = None, plan_name: Optional[str] = None):
    ...
    if project_root and task:
        ...
        if task_branch_has_no_changes(project_root, task.id, spec, plan_name):
...
def _print_halt_followup(halt_payload: dict, session_id: Optional[str]) -> None:
    ...
    if task_id:
        ...
        if final_decision not in ("completed", None):
            # followup.halt_followup names `snodo authorize` only when a
            # decision is actually pending, and the retry otherwise (Fixes #288).
