...
def _post_snapshot(snapshot: dict) -> None:
    """PUT the snapshot to ``{api_url}/live/{session_id}``. Drop on any failure.
    ...
    url = f"{api_url.rstrip('/')}/live/{quote(snapshot['session_id'], safe='')}"
