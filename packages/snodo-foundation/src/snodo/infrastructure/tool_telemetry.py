...
def _record_drop(
    ...
    if len(_dropped_records) < _MAX_DROPPED_RECORDS:
        _dropped_records.append(drop_event)
    _logger.debug(
    ...
    if project_root:
        _persist_drop(project_root, drop_event)


def _persist_drop(project_root: str, drop_event: dict) -> None:
    """Safely persist a dropped telemetry event under project_root/.snodo."""
    try:
        ...
        target_id = drop_event.get("target_id") or ""
        rec = drop_event.get("record")
        job_id = target_id if target_id and target_id.startswith("j_") else None
        task_id = None
        if target_id and target_id.startswith("task_"):
            task_id = target_id
        elif isinstance(rec, dict) and rec.get("task_ref") and rec.get("task_ref") != "unknown" and str(rec.get("task_ref")).startswith("task_"):
            task_id = str(rec["task_ref"])

        def _update_target_drops(state: dict) -> None:
            drops = state.get("tool_telemetry_drops", [])
            if not isinstance(drops, list):
                drops = []
            drops.append(drop_event)
            state["tool_telemetry_drops"] = drops

        if job_id:
            atomic_update_json(root_path / ".snodo" / "jobs" / job_id, "state.json", _update_target_drops)
        if task_id:
            atomic_update_json(root_path / ".snodo" / "tasks" / task_id, "state.json", _update_target_drops)

        def _update_global_drops(data: dict) -> None:
        ...
        _logger.debug("Failed to persist telemetry drop to %s: %s", project_root, e)
...
def persist_tool_telemetry(target_id: str, record: dict) -> None:
    ...
    if not record or not isinstance(record, dict):
        _record_drop("invalid_record", target_id, record, "Record is empty or not a dict", project_root=project_root)
        return
    ...
    if not job_id and not task_id:
        _record_drop("no_target_id", target_id, record, "No resolvable job_id or task_id", project_root=project_root)
        return

    if job_id:
        _append_telemetry(Path(project_root) / ".snodo" / "jobs" / job_id, record, project_root=project_root)
    if task_id:
        _append_telemetry(Path(project_root) / ".snodo" / "tasks" / task_id, record, project_root=project_root)


def _append_telemetry(target_dir: Path, record: dict, project_root: str | None = None) -> None:
    ...
    try:
        ...
        if _is_system_root_or_temp(target_dir.parent.parent):
            _record_drop("target_in_system_or_temp", str(target_dir), record, "Target directory parent is system or temp", project_root=project_root)
            return
        ...
        _record_drop("append_exception", str(target_dir), record, str(e), project_root=project_root)
