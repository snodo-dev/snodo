...
class SerdeMixin:
    ...
    def _dict_to_state(self, d: Dict[str, Any]) -> LoopState:
        ...
        task = Task(
            ...
            attempt_reads=task_dict.get("attempt_reads", []),
            depth=task_dict.get("depth", 0),
            flow_type=task_dict.get("flow_type"),
            wave_id=task_dict.get("wave_id"),
            plan_name=task_dict.get("plan_name"),
            plan_wave=task_dict.get("plan_wave"),
        )
        ...
        return LoopState(
            ...
            spawned_subtasks=[
                Task(
                    ...
                    prior_failures=s.get("prior_failures", []),
                    attempt_provenance=s.get("attempt_provenance", []),
                    attempt_reads=s.get("attempt_reads", []),
                    depth=s.get("depth", 0),
                    flow_type=s.get("flow_type"),
                    wave_id=s.get("wave_id"),
                    plan_name=s.get("plan_name"),
                    plan_wave=s.get("plan_wave"),
                )
    ...
    def _state_to_dict(self, state: LoopState) -> Dict[str, Any]:
        ...
        return {
            "task": {
                ...
                "prior_failures": state.task.prior_failures,
                "attempt_provenance": state.task.attempt_provenance,
                "attempt_reads": state.task.attempt_reads,
                "depth": state.task.depth,
                "flow_type": state.task.flow_type,
                "wave_id": state.task.wave_id,
                "plan_name": state.task.plan_name,
                "plan_wave": state.task.plan_wave,
            },
            ...
            "spawned_subtasks": [
                {
                    ...
                    "attempt_provenance": s.attempt_provenance,
                    "attempt_reads": s.attempt_reads,
                    "depth": s.depth,
                    "flow_type": s.flow_type,
                    "wave_id": s.wave_id,
                    "plan_name": s.plan_name,
                    "plan_wave": s.plan_wave,
                }
