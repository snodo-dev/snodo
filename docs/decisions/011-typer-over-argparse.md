...
- **Decision**: Replace argparse-based CLI with Typer (`main.py:13,33-37`). The root `app` registers the top-level commands and sub-apps (plan, job, agent, config, session, mode). The `main()` entry point (`main.py:555-571`) accepts an optional `argv` parameter for test invocation.
