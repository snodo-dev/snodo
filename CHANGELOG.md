...
- `snodo merge` conflict messaging updated to accurately reflect that aborted merges leave the base branch clean, and now lists all conflicting file paths. Previously, `GitMCP.merge_branch` executed `git merge --abort` on conflict to keep the base branch clean, but the CLI printed "The branch and worktree were left intact for manual resolution. Resolve it, then re-run `snodo merge` to continue." — falsely claiming unmerged conflicts were present in the working tree index when `git ls-files -u` was empty. `MergeConflictError` now collects conflicting paths prior to aborting, and the CLI explicitly names the conflicting paths, explains that the merge was rolled back to leave the base branch clean, and outputs the exact command (`git merge <branch>`) to run for manual resolution.
