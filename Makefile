...
#                    file-length ratchet, docs-coverage ratchet
...
gate: gate-init _gate-push
	@ssh $(GATE_HOST) '$(GATE_PATH); cd $(GATE_DIR) && uv sync --all-extras -q && uv run pytest tests/ -q -n $(GATE_JOBS) && uv run ruff check . && uv run lint-imports && uv run python scripts/enforce_file_length.py && uv run python scripts/enforce_docs_coverage.py'
